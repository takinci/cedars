...
| AJR-2025-CT | Hehenkamp G et al. *CT Energy Consumption Savings From a Rapid-Reactivation Power-Save Mode for Interexamination Idle Periods.* AJR 2025. DOI: [10.2214/AJR.25.33951](https://doi.org/10.2214/AJR.25.33951). **Corrected 2026-08** — previously mischaracterized as a "review"; it's a direct power-meter study (1-second sampling, 124 workdays/28 weeks) on one real Siemens Somatom X.ceed CT (University Hospital Basel). Reports actual hours-in-mode, not just power draw: of 1,253 h total "on" time, 425 h active (33.9%), 347 h idle (27.7%), 481 h power-save (38.4%) — the first real duty-cycle (not just power) data found for CT. Note this is a percentage of *on*-time only, not of the full calendar period, and n=1 scanner/site — see the `off_h` hour-allocation note below for how this and Radiol-253128 inform (but don't yet correct) the CT row. |
...
| ID | Citation |
|----|---------|
| EurRad-2026-Mammo | Rossini G, Lockey K, Rokoszak M, et al. *Energy consumption of standard and contrast-enhanced mammography: a step towards sustainable breast imaging.* Eur Radiol 2026;36:5607–5615. DOI: [10.1007/s00330-026-12373-2](https://doi.org/10.1007/s00330-026-12373-2). First systematic direct-measurement (minute-by-minute power monitoring) study of mammography energy — 3 real units, 2 vendors, 193 exams. **Verified 2026-08** (the DOI looked malformed on first pass since no other *European Radiology* citation in this document uses a 4-digit `-026-` year fragment, but it's a real, recently-published paper — confirmed by reading it directly). **Primary source for the corrected mammography `EQUIPMENT_UNITS` defaults below.** |

**Mammography `EQUIPMENT_UNITS` defaults — corrected 2026-08 (active 5→0.5 kW, idle 1→0.2 kW, standby 0.3→0.15 kW, off unchanged at 0.1 kW; implied annual total ≈10,190→≈1,850 kWh/yr):** the prior defaults were never checked against a direct mammography power measurement — none existed until Rossini et al. 2026. That paper reports two things CEDARS's 4-state model doesn't map to directly: **net (incremental exposure) energy of only 0.05–0.09 kWh/exam** — the x-ray exposure itself is brief and low-power, unlike CT/MRI — and **annual per-machine totals of ~1,660–2,300 kWh/yr**, dominated by idle/standby baseload rather than active scanning (consistent with the CT/MRI/workstation pattern found elsewhere in this document: nonproductive time, not active use, drives most equipment energy). The corrected values are back-solved to reproduce that real annual range at this table's own hour split (100/250/300/94 active/idle/standby/off h-month) and scan volume (800/month): `active_kw` down 10× to ≈0.5 kW (0.05–0.09 kWh over a 4–6 min exam implies well under 1 kW average, even with brief spikes to 0.5–3.5 kW during the exposure itself per the paper's Fig 1), `idle_kw`/`standby_kw` down to a modest step-down (0.2/0.15 kW) consistent with a blended non-active baseload of ≈0.16–0.17 kW that reproduces the measured annual total; `off_kw` (0.1 kW) was already in a plausible low range and left unchanged. This is a **derived** fit, not a direct per-state measurement (the paper measures net-vs-baseload, not CEDARS's active/idle/standby/off split), so treat it as much-better-grounded than the prior default rather than a final authority — override via **Advanced equipment parameters** with your own metered figures if available.
...
| MOU-2025 | Vosshenrich J, Merkle EM, Heye T. *The carbon footprint of modern imaging.* Curr Opin Urol 2025;35:674–678. DOI: [10.1097/MOU.0000000000001337](https://doi.org/10.1097/MOU.0000000000001337). **Corrected 2026-08** — previously misattributed as "Vosshenrich R et al." (2024); the actual first author is Jan Vosshenrich, and the paper's own header dates it Volume 35, Number 6, **November 2025**, not 2024. **Verified 2026-08 by reading the full text**: it directly states global average electricity carbon intensity "0.473 kg CO₂e/kWh" and EU average "0.237 kg CO₂e/kWh" — both exact matches to CEDARS's defaults — and ultrasound "~2500 kWh of electricity per year," an exact match to the `MODALITY_BENCHMARKS` ultrasound row. Primary source for `MODALITY_BENCHMARKS` annual kWh values displayed in the Dashboard Infrastructure section and for the global/EU carbon intensity defaults cited throughout the UI. **The benchmark's energy (kWh/yr) is shown as reported by each source publication; the CO₂ column is not stored but derived at render from a single common factor (`BENCHMARK_CI` = global average 0.473 kgCO₂e/kWh) so rows are directly comparable — the source publications themselves reported CO₂ at varying local grids (~0.20–0.24). This benchmark table is a reference display only and does not feed the footprint calculation, which always uses the user's selected regional grid factor.** |
| Schoen-CT-Idle | **Resolved 2026-08.** Brown M, Schoen JH, Gross J, Omary RA, Hanneman K. *Climate change and radiology: Impetus for change and a toolkit for action.* Radiology 2023;307:e230229. DOI: [10.1148/radiol.230229](https://doi.org/10.1148/radiol.230229). Confirmed by reading the paper directly: it states equipment was used inefficiently "approximately two-thirds for CT and one-third for MRI" when not in active use — a direct match for the `MODALITY_BENCHMARKS` CT row's "idle up to 66% of total" note. **However, this claim is itself secondhand** — the paper cites it to its own reference 16, which is **Heye et al. 2020** (`Radiol-2020-Heye`, already cited above in the MRI section for the same "one-third of MRI energy in off mode" figure). So Heye et al. 2020 is the true primary source for both the CT-66% and MRI-33% idle-share figures; this 2023 "toolkit" paper is a secondary citation vehicle, not independent corroboration. |

> ⚠️ **`MODALITY_BENCHMARKS` MRI 1.5T row (269,400 kWh/yr) flagged, not yet corrected — checked against MOU-2025's actual text 2026-08.** MOU-2025 states "state-of-the-art MRI scanners require approximately 80,000–170,000 kWh annually" as a single range (not broken out by field strength in that paper). CEDARS's `MRI 3T` row (125,000 kWh/yr) sits inside that range, but `MRI 1.5T` (269,400 kWh/yr) is **~1.6× above the paper's own upper bound** — and physically counter-intuitive, since 1.5T scanners are not typically higher-consuming than 3T. The row's note attributes it jointly to "Vosshenrich; Chaban 2023," so it isn't solely MOU-2025's number to begin with; not changed here since it's unclear which blended source the 269,400 figure was meant to represent, and this benchmark table is display-only (doesn't feed the footprint calculation) — but worth a dedicated check against Chaban et al. 2023's own figures before trusting this row.
...
> ⚠️ **Citation corrected 2026-08 — the org name was wrong, but a real census exists and is now confirmed in hand.** No "British Institute of Radiology Radiology Workforce Census" exists — BIR's actual publications are journals (*BJR*, *DMFR*, etc.). The real, annual UK radiology workforce census (16th edition as of 2023, 100% response rate from all 159 UK acute trusts/health boards) is published by the **Royal College of Radiologists** (`RCR-Census-2023`, a different organisation), confirmed 2026-08 by reading the actual document. NHS England's Diagnostic Imaging Dataset remains unrelated — it records exam volumes and waiting times, no staffing data.
>
> Having now read the full RCR census, it confirms what the earlier research pass found by other means: it reports workforce data at **national/aggregate level** (total consultants, vacancy rates, growth by subspecialty, regional shortfalls) and does **not** break staffing down by device-unit for most modalities — there is no CT/MRI/PET-CT/fluoroscopy/ultrasound/mammography per-scanner FTE figure anywhere in it. **One genuine exception: interventional radiology.** The census states a fully effective IR service requires *"a 24/7 1:6 IR rota (six consultants working on the rota)"* (p.24, "Interventional radiology services") — a real, specific, directly relevant number that lines up with CEDARS's `angio` row (6 FTE/unit), though it describes **consultant rota coverage for a 24/7 service**, not literally "6 total departmental FTE including nurses/techs/scrub" the way the row's description implies — so treat it as corroborating context for the row being in the right range, not a validating measurement of the exact figure. All other rows remain uncited: MRI (all field strengths), CT, PET-CT, fluoroscopy, radiography, ultrasound, mammography, PACS, and workstations have no per-device FTE figure in this census or anywhere else located in the 2026-08 research pass (which also checked a 2025 radiographer-staffing paper, EU-REST, that deliberately avoided publishing fixed per-device ratios, and AAPM Report 33's physicist-only — not whole-department — modality ratios).
>
> **Conclusion: only the Angio/IR row now has a real, if partial, citation; the rest remain illustrative, uncited expert-estimate defaults.** Replace with your own department's actual HR/rostering data via local reporting.
...
**Per-modality file sizes (`MODALITY_MB`, MB/study).** **Corrected 2026-08** — this was previously conflated with the "Implementation Guide" citation (`Doo-JACR-2024`, DOI `...jacr.2023.11.019`, JACR 2024;21:239–247). It's actually a **different** paper by the same lead author in the **same JACR 2024 issue**: Doo FX, Kulkarni P, Siegel EL, et al. *Economic and Environmental Costs of Cloud Technologies for Medical Imaging and Radiology Artificial Intelligence.* J Am Coll Radiol 2024;21:248–256. DOI: [10.1016/j.jacr.2023.11.011](https://doi.org/10.1016/j.jacr.2023.11.011) (`Doo-JACR-2024-Cloud`) — confirmed as a real, correctly-titled paper via direct search, though the specific Fig 2 figures below have not been independently re-verified against its full text in this pass. Fig 2: XR ~10, US ~300, CT ~300, MR ~300, mammography ~10, tomosynthesis ~400, PET-CT ~1700 MB. CT is set to ~700 MB to represent a study **with reformats** (Jia et al., below, median CT-CAP 787 MB all-series vs 290 MB axial-only). All values are editable estimates; file sizes vary widely by site, protocol, slice thickness, and compression.
