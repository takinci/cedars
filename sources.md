...
**CT `EQUIPMENT_BASE` defaults — corrected 2026-08 (3 kW active, 1.5 kW idle, 1 kW standby, 0.5 kW off; was 60 / 8 / 3 / 0.2 kW):** the prior default cited Acra-2024 and CJRS-2022 as supporting "40–80 kW active." Checking both directly, neither reports anything near that for any state they measure: Acra-2024 (Schulz et al., a 3-scanner dual-energy-CT study on *low-utilization-time* power) reports idle ≈2.6 kW, low-power ≈0.89 kW, off <0.01 kW; CJRS-2022 (Brown et al., a real Siemens dual-source 128-slice CT) reports "System ON" (powered, non-scanning) ≈3 kW, "Computer ON" idle ≈1.5 kW, full shutdown ≈0.5 kW. The likely explanation for the old default: 40–80 kW is a peak/instantaneous X-ray-tube-exposure power spec (correct for the few seconds a tube actually fires) misapplied as a *sustained* active-hours average — which alone would explain roughly the ~4–5× CT overestimate found in independent validation against Thiel et al. 2024's reported footprint.
...
| Radiol-230441 | Woolen SA et al. *Ecodesign and Operational Strategies to Reduce the Carbon Footprint of MRI for Energy Cost Savings.* Radiology 2023;307:e230441. DOI: [10.1148/radiol.230441](https://doi.org/10.1148/radiol.230441). Direct power-meter measurements (1 Hz sampling, 39 days) on 4 real outpatient MRI scanners across 3 vendors, segmented into 5 modes: **off 7.3–9.7 kW, idle 9.5–14.5 kW, prepared-to-scan 17.3–25.6 kW, scan 28.6–48.3 kW, power-save 6.4 ± 0.1 kW**. Projected annual energy 82.7–171.1 MWh/scanner, 72–91% classified nonproductive. **Primary source for the corrected MRI `off_kw` values** below (a 5-mode measurement doesn't map 1:1 onto CEDARS's 4-state active/idle/standby/off model, so only the clearest, most physically-grounded finding — off ≈ most of idle, not near-zero — was incorporated; the full active/idle/standby table remains anchored to Heye/Vosshenrich/Klein as before, since that set is independently self-consistent against its own annual-kWh calibration targets). |
...
**MRI `EQUIPMENT_BASE` defaults (30 kW active, 15 kW idle, 5 kW standby):** JMRI-2023 (Heye et al.) reports a mean of 30.1 kW for active 3T scanners and substantial idle draw even when not scanning. idle_kw ≈ 50 % of active is consistent with Radiol-243453. Cryocooler draw contributes to standby. Replace with scanner logs via the **Advanced equipment parameters** override — see "Measured-data equipment overrides" below. The default `scans`/month per device is a literature-informed illustrative volume, not a site-specific estimate; when validating against a specific department's reported annual study counts (any modality), enter that department's actual volume through the same override mechanism rather than relying on the default.

**MRI `off_kw` for superconducting field strengths (1.5T/3T/7T) — corrected 2026-08 (1.5 kW→24 kW, 0.5 kW→11.25 kW, 1.0 kW→16.5 kW):** the prior near-zero `off_kw` values were physically implausible for any cryogen-cooled MRI. A superconducting magnet's cryocooler must run continuously — switching it off risks helium boil-off and magnet quench (a multi-day, five/six-figure-cost event) — so "off" state still draws most of what "idle" does. Direct power-meter data (Woolen et al. 2023, Radiol-230441, above) measured off at 65–100% of idle power across 4 real scanners; a 75% ratio was applied here. Because CEDARS allocates only 34 h/month to the off state, this only shifts each variant's annual total by +3–4% — small enough not to invalidate the existing Heye/Vosshenrich/Klein-derived annual-kWh calibration targets it's layered on top of. `mri_035t` (permanent magnet, no cryocooler) is unaffected — its existing "idle ≈ off" assumption already holds physically.
...
| ID | Citation |
|----|---------|
| Radiol-240398 | Thiel CL et al. *Environmental Life Cycle Assessment of a U.S. Hospital-based Radiology Practice.* Radiology 2024. DOI: [10.1148/radiol.240398](https://doi.org/10.1148/radiol.240398). **This is the same paper Thiel et al. used as the primary validation benchmark elsewhere in this repo's development history** — a full department-scale LCA (Vanderbilt University Medical Center), not a generic IT-infrastructure paper. Reports reading-room workstation power directly: **170 W per station, 60% daily use** (their own modelling assumption, per their Appendix S1). Also reports that 25 of 27 PACS reporting stations were left on overnight/weekends unnecessarily (~$7,253/yr wasted, ~10 cars' worth of CO₂e/yr) — evidence the paper's own workstation estimate is well below what CEDARS previously assumed. Source for **workstation** defaults only — see below for why it does not support the PACS/servers default. |
...
**Workstations `EQUIPMENT_UNITS` default — corrected 2026-08 (was 2 kW / 0.8 kW / 0.2 kW / 0.05 kW, now 0.17 kW / 0.068 kW / 0.017 kW / 0.005 kW):** the prior default overstated Thiel-2024's own reported figure (170 W) by **~12×** — most likely a straightforward transcription error when the default was first entered (the paper is unambiguous: "170-W power consumption for the onsite reading room workstations, with 60% use per day"), not a legitimate alternative estimate or different methodology. `active_kw` (0.17) is now a direct, high-confidence match to the source. Thiel's own model is a **simpler 2-state one** (170 W at 60% duty vs. effectively off) — it does not report separate idle/standby/off figures, so those three fields here are the *old* default's idle/standby/off proportions re-scaled onto the corrected 170 W anchor, not independently verified against Thiel or any other source. Treat `idle_kw`/`standby_kw`/`off_kw` as lower-confidence than `active_kw` and override via **Advanced equipment parameters** if you have metered data.

**PACS/servers `EQUIPMENT_UNITS` default (4 kW, near-constant across all power states) — recited 2026-08, number unchanged.** Previously cited to Radiol-240398 (Thiel et al.), but nothing in that paper's reported figures (the 170 W workstation modelling assumption, or the $7,253/yr overnight-waste finding, which back-calculates to roughly 300–400 W of *wasted* overnight/weekend draw per station across the 25 PACS stations, not 4 kW) supports a 4 kW rack/server-set baseline — that citation was wrong and has been removed. Checked against general (non-medical-specific) data-center literature instead: a single fully-populated 42U server rack typically draws 3–5 kW (up to 10–15 kW for high-performance configurations), so 4 kW is a **plausible, unremarkable figure for one modest server/switch closet** — not contradicted the way the old CT and workstation defaults were. No medical-PACS-specific power study was found to anchor this more precisely, so treat it as a coarse, low-confidence general-IT estimate rather than a validated radiology-specific figure, and override via **Advanced equipment parameters** if you know your actual server count/specs.
...
| Doo-2024 | Doo FX et al. *Environmental Sustainability and AI in Radiology: A Double-Edged Sword.* Radiology 2024. DOI: [10.1148/radiol.232030](https://doi.org/10.1148/radiol.232030). (See AI sustainability section.) Discusses contrast media as part of radiology's environmental footprint — iodinated/gadolinium contrast persistence in water and stewardship — within the broader sustainability framework. **[verify specific contrast figures against the paper]** |
...
**Storage energy intensity (`STORAGE_KWH_PER_TB_ONPREM = 600`, `STORAGE_KWH_PER_TB_CLOUD = 360` kWh/TB/yr).** Jia Y et al., *Eur Radiol* 2026 (DOI: [10.1007/s00330-025-12023-z](https://doi.org/10.1007/s00330-025-12023-z)): hardware-centric model — HDD 136 kWh/TB/yr + servers (2059 kWh/server ÷ 12 TB) + network (0.1 device/server × 3197 kWh/device ÷ 12 TB) ≈ 335 kWh/TB/yr IT, × PUE 1.8 ≈ 600 on-premises; efficient cloud ≈ 40% lower (their conservative cloud estimate; higher estimates reach 80%). This is a **small, dense-compute rack** architecture (12 TB/server) using drive figures reflecting older/smaller-capacity HDDs.

There is **no single "correct" replacement constant** — storage kWh/TB spans roughly two orders of magnitude in the literature depending on drive generation and server density, not just architecture "type." The 136 kWh/TB HDD component is the dominant term (~95% of the pre-PUE IT total) and is itself drive-generation-dependent: modern high-capacity helium drives run as low as **~0.3 W/TB** (≈2.6 kWh/TB/yr) — roughly 50× more efficient than the drives implicit in the 136 kWh/TB figure. Re-deriving Jia's own formula at a realistic bulk-archive density (e.g. ~448 TB/server) with modern-drive HDD efficiency and a lower PUE (~1.3, typical of a modern archive facility rather than a general server room) gives **≈6–15 kWh/TB/yr** — squarely matching Thiel et al. 2024's implied storage intensity (~6.75 kWh/TB/yr) for their reported department. Server *density* alone (re-scaling just the 12 TB/server term to 448 TB/server, keeping the 136 kWh/TB HDD figure unchanged) only gets to ~170–250 kWh/TB/yr — drive generation, not server count, is the dominant lever.

**Practical guidance:** the default (600) remains a legitimate, conservative reference for Jia et al.'s own architecture (small server count, older/smaller drives) — appropriate when you don't know your actual hardware. For a modern bulk-archive PACS deployment (large helium/SSD drives, high server density, efficient facility PUE), enter **~10–15 kWh/TB/yr** as a literature-informed starting point via the **Custom intensity (kWh/TB/yr)** field (Home page, Data storage & archiving section) rather than the 600 default — then replace with your own metered figure (from facility/cloud invoices) for reporting of record. This is a *derived* estimate (component figures re-combined from Jia et al.'s formula, not a single paper's end-to-end reported number), so treat it as directional, not authoritative.
