...
**CT `EQUIPMENT_BASE` defaults (60 kW active, 8 kW idle, 3 kW standby):** supported by Acra-2024 and CJRS-2022 reporting typical modern multi-detector CT active draws of 40–80 kW and significant idle consumption; 60 kW is a mid-range default representing modern multi-detector CT — a pre-~2015 or single/dual-detector scanner should be materially lower. Replace with scanner-specific power meters or OEM data sheets via the **Advanced equipment parameters** override (Home page, per device type you've added) — see "Measured-data equipment overrides" below.
...
| ID | Citation |
|----|---------|
| Radiol-240398 | See CT section. Radiology 2024. IT/PACS infrastructure power; the only literature source underlying both the PACS/servers and workstation `EQUIPMENT_UNITS` defaults below — a coarser evidence base than the dedicated per-modality scanner literature above. |

**PACS/servers `EQUIPMENT_UNITS` default (4 kW, near-constant across all power states):** a rack/server-set baseline from Radiol-240398; near-constant because server/storage infrastructure doesn't power down like a scanner. Virtualisation and server consolidation can reduce this by 20–40% (Doo-2024).

**Workstations `EQUIPMENT_UNITS` default (2 kW active, 0.8 kW idle, 0.2 kW standby, 0.05 kW off):** a per-reading-station estimate from Radiol-240398, covering a diagnostic workstation with multi-monitor array (typically 2–3× 4–6MP medical-grade displays) plus the local workstation PC — materially higher draw than a single consumer office PC/monitor. This is the **least independently verified** default in the equipment table: it rests on one multi-modality infrastructure paper rather than a dedicated workstation power-measurement study, unlike every scanner modality above. Departments with lower-spec reading setups, laptop-based reporting, or thin-client/virtualised desktops should expect substantially lower per-station draw and are encouraged to meter and override rather than rely on this default.
