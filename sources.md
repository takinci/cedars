...
**PACS/servers `EQUIPMENT_UNITS` default (4 kW, near-constant across all power states) — unverified, flagged for review:** also cited to Radiol-240398, but nothing in that paper's reported figures (the 170 W workstation modelling assumption, or the $7,253/yr overnight-waste finding, which back-calculates to roughly 300–400 W of *wasted* overnight/weekend draw per station across the 25 PACS stations, not 4 kW) supports a 4 kW rack/server-set baseline. Unlike the workstation fix above, there isn't a single clean number in this paper to swap in for "PACS/servers" as CEDARS defines that row (server/switch infrastructure separate from reading stations) — this default should be treated as **unverified** until a dedicated source is found or a user supplies a measured value via **Advanced equipment parameters**.
...
**Storage energy intensity (`STORAGE_KWH_PER_TB_ONPREM = 600`, `STORAGE_KWH_PER_TB_CLOUD = 360` kWh/TB/yr).** Jia Y et al., *Eur Radiol* 2026 (DOI: [10.1007/s00330-025-12023-z](https://doi.org/10.1007/s00330-025-12023-z)): hardware-centric model — HDD 136 kWh/TB/yr + servers (2059 kWh/server ÷ 12 TB) + network ≈ 335 kWh/TB/yr IT, × PUE 1.8 ≈ 600 on-premises; efficient cloud ≈ 40% lower (their conservative cloud estimate; higher estimates reach 80%). This is a **small, dense-compute rack** architecture (12 TB/server); a **large bulk-archive array** (hundreds of TB/server, little or no PUE overhead) is a structurally different storage class with a much lower kWh/TB — the two are not directly comparable. A **Custom intensity (kWh/TB/yr)** field on the Home page (Data storage & archiving section) fully overrides the on-prem/cloud default with a site- or architecture-specific figure — use it when validating against, or reporting for, a storage architecture that doesn't match the Jia et al. reference.
