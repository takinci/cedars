...
describe('computeDashboard — FLEET, Germany, Annual', () => {
  ...
  it('total energy, carbon, and energy-per-scan', () => {
    expect(d.totals.kwh).toBeCloseTo(735331.92, 2);
    expect(d.totals.co2Kg).toBeCloseTo(264719.49, 1);
    expect(d.totals.energyPerScan).toBeCloseTo(3.583, 3); // kWh ÷ scans
  });
  ...
  it('GHG scopes: Scope 2 = operational carbon; Scope 3 travel = scans × 20 km × 0.17 kg/km', () => {
    expect(d.scopes.scope2Kg).toBeCloseTo(264719.49, 1);
...
describe('computeInterventions — FLEET, delta from current config', () => {
  it('two operational levers stack additively on energy', () => {
    ...
    expect(iv.baseline.kwh).toBeCloseTo(735331.92, 2);
    expect(iv.savings.kwh).toBe(21120);              // overnight 11520 + low-value 9600
    expect(iv.projected.kwh).toBeCloseTo(714211.92, 2);
  ...
  it('storage axial-only lever saves the reformats delta vs current all-reformats config', () => {
    ...
    expect(iv.savings.pctEnergy).toBeCloseTo(14.8, 1);
  });
